...
def cfg() -> FlowConfig:
    ...
    # СТАРЫЙ разворот («продать и сразу купить обратную на перекрытие долга»).
    # Он остался в коде и покрыт этими тестами; по умолчанию же теперь
    # работает новый — разворот только ЗАКРЫВАЕТ ногу, а обратная сторона
    # обязана заново пройти все ворота. Новое поведение проверяет
    # TestLadderReenters.
