...
class FlowConfig:
    ...
    # МИНИМАЛЬНЫЙ РАЗМЕР ОРДЕРА. Шэры округляются до сотых, и округление вниз
    # опускало КАЖДУЮ покупку под доллар: $1.00 / 0.53 = 1.8867 -> 1.88 шэра
    # -> $0.9964. Теперь размер добивается вверх до этого порога.
    jump_min_order_usdc: float = 1.0
    ...
    #
    jump_tp_trail: float = 0.02
    # НА СКОЛЬКО надо быть в плюсе, чтобы стоп вообще проснулся.
    #
    # Раньше взвод был привязан к jump_tp_min_gain (10¢), и это и было
    # настоящей блокировкой: бид обязан был превысить УПЛАЧЕННЫЙ ask на 10¢,
    # то есть подняться на 10¢ плюс весь спред. Пока этого не случалось,
    # позиция падала от пика на сколько угодно без единой реакции — трейлинг
    # выглядел сломанным, хотя просто ни разу не включался.
    #
    # Совсем без взвода нельзя: сразу после покупки бид ниже ask на спред,
    # и стоп срабатывал бы в тот же тик на каждой сделке.
    jump_tp_trail_arm: float = 0.02
    # ЖЁСТКИЙ СТОП: процент ушёл ниже нашего входа — выходим, не дожидаясь
    # ни лестницы, ни трейлинга.
    #
    # Отсчёт от БИДА НА ВХОДЕ, а не от уплаченного ask. Это не придирка:
    # спред означает, что сразу после покупки бид уже ниже ask, и стоп от ask
    ...
    # ЧТО ГЛАВНЕЕ — РАЗВОРОТ ИЛИ ВЫХОД. Порог стопа (1¢) меньше порога
    # лестницы (2¢ + выдержка 0.6с), поэтому по цене стоп наступает раньше
    # ВСЕГДА, и лестница при включённом стопе почти не случалась. Поменять
    # порядок вызовов мало — нужно, чтобы стоп СТОЯЛ В СТОРОНЕ, пока ногу
    # способен взять разворот. Тогда стоп остаётся страховкой ровно на те
    # случаи, когда развернуться не во что: лестница выключена, упёрлась в
    ...
    # она противоречит самой сути стопа: стоп нужен, чтобы ВЫЙТИ на падении,
    # а пол запрещает продавать именно тогда, когда падает сильнее всего.
    # Чем быстрее рушится цена, тем упорнее бот держал бы ногу — то есть
    # ровно наоборот. В бою нога съехала с 0.47 до 0.10 при стопе на 0.45.
    #
    # Ненулевое значение имеет смысл только для ФИКСАЦИИ ПРИБЫЛИ, где ждать
    # действительно можно. Для стопа — никогда.
    jump_max_sell_slip: float = 0.0
    ...
    # Сколько отказов продажи подряд терпеть, прежде чем снять ногу с
    # торговли. Выход не придерживается паузой после ошибки (стоп обязан
    # срабатывать сразу), поэтому устойчивый отказ иначе повторяется на
    # каждом такте — десять раз в секунду. 0 = терпеть бесконечно.
    jump_max_sell_fails: int = 5
    ...
    def from_env(cls) -> "FlowConfig":
        ...
        return cls(
            ...
            jump_ladder_before_stop=_b("JUMP_LADDER_BEFORE_STOP", True),
            ...
            jump_max_sell_fails=_i("JUMP_MAX_SELL_FAILS", 5),
